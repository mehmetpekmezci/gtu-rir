...
def compute_generator_loss(epoch,netD,real_RIRs, fake_RIRs, real_labels, conditions,filters, gpus,mfcc_transform_fn,cfg):
    ...
    ### MPEKMEZCI SSIMONLY
    #MSE_error1 = 0
    #MSE_error1 = loss(mfcc_transform_fn(real_RIRs[:,:,0:3968]),mfcc_transform_fn(fake_RIRs[:,:,0:3968]))
    MSE_error2 = loss1(real_RIRs[:,:,3968:4096],fake_RIRs[:,:,3968:4096])
    

    ## MPEKMEZCI SSIM(MFCC)
    #SSIM=audio_ssim(real_RIRs[:,:,0:3968],fake_RIRs[:,:,0:3968],mfcc_transform_fn)
    #SSIM_LOSS=1-SSIM
    #SSIM_LOSS=0
    #print("MSE_error1:"+str(MSE_error1))
    #print("SSIM_LOSS:"+str(SSIM_LOSS))
    #MSE_error1 = MSE_error1 + SSIM_LOSS
