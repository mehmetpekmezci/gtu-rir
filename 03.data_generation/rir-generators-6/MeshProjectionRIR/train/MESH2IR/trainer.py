...
#        print(netD)
        ...
        summary(netG,[(self.batch_size,SOURCE_RECEIVER_XYZ_DIM),(self.batch_size,int(2*4*cfg.IMAGE_RESOLUTION/8*cfg.IMAGE_RESOLUTION/8))] )
...
#                    param_group['lr'] = discriminator_lr
            ...
            for i, data in enumerate(data_loader, 0):
              ...
              try: 
                ...
                if mesh_embedding_source_image is None:
                    continue
                ...
                ######################################################
                with torch.no_grad():
                     mesh_embedding_source_image_latents=image_vae.encode(mesh_embedding_source_image).latent_dist.sample().reshape(self.batch_size,int(4*cfg.IMAGE_RESOLUTION/8*cfg.IMAGE_RESOLUTION/8))
                     mesh_embedding_receiver_image_latents=image_vae.encode(mesh_embedding_receiver_image).latent_dist.sample().reshape(self.batch_size,int(4*cfg.IMAGE_RESOLUTION/8*cfg.IMAGE_RESOLUTION/8))
