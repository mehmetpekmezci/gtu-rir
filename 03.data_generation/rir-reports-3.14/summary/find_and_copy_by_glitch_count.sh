...
then
	   ...
	   N=$(ls $BUT_RIR_DATA_DIR/$REF_METRIC_ROOM/MicID*/SpkID0$PHYSICAL_SPRAKER_NO*/$MICROPHONE_NO/RIR/IR_sweep_15s_45Hzto22kHz_FS16kHz.v00.wav | wc -l)
	   if [ $N -gt 1 ]
	   then
	       cp $BUT_RIR_DATA_DIR/$REF_METRIC_ROOM/MicID*/SpkID0$PHYSICAL_SPRAKER_NO*S/$MICROPHONE_NO/RIR/IR_sweep_15s_45Hzto22kHz_FS16kHz.v00.wav $SUMMARY_DIR/real.rir.$REF_METRIC_ROOM-micstep-$MICROPHONE_ITERATION-spkstep-$SPEAKER_ITERATION-spkno-$PHYSICAL_SPRAKER_NO-micno-$MICROPHONE_NO.wav
           else
	       cp $BUT_RIR_DATA_DIR/$REF_METRIC_ROOM/MicID*/SpkID0$PHYSICAL_SPRAKER_NO*/$MICROPHONE_NO/RIR/IR_sweep_15s_45Hzto22kHz_FS16kHz.v00.wav $SUMMARY_DIR/real.rir.$REF_METRIC_ROOM-micstep-$MICROPHONE_ITERATION-spkstep-$SPEAKER_ITERATION-spkno-$PHYSICAL_SPRAKER_NO-micno-$MICROPHONE_NO.wav
           fi
fi
