...
def get_ray_directions(RESOLUTION_QUOTIENT=1):
...
def generate_ray_centers(mesh,NUMBER_OF_RAY_CENTERS=10):
   RATIO_OF_PROXIMITY_TO_OUTER_WALLS=0.85
   MAX_X=np.max(np.array(mesh.vertices[:,0]))
   MIN_X=np.min(np.array(mesh.vertices[:,0]))
   MAX_Y=np.max(np.array(mesh.vertices[:,1]))
   MIN_Y=np.min(np.array(mesh.vertices[:,1]))
   MAX_Z=np.max(np.array(mesh.vertices[:,2]))
   MIN_Z=np.min(np.array(mesh.vertices[:,2]))
   randomRayCenterPositions=np.random.rand(NUMBER_OF_RAY_CENTERS,3)
   randomRayCenterPositions[:,0]=(randomRayCenterPositions[:,0]*(MAX_X-MIN_X)+MIN_X)*RATIO_OF_PROXIMITY_TO_OUTER_WALLS
   randomRayCenterPositions[:,1]=(randomRayCenterPositions[:,1]*(MAX_Y-MIN_Y)+MIN_Y)*RATIO_OF_PROXIMITY_TO_OUTER_WALLS
   randomRayCenterPositions[:,2]=(randomRayCenterPositions[:,2]*(MAX_Z-MIN_Z)+MIN_Z)*RATIO_OF_PROXIMITY_TO_OUTER_WALLS
   return randomRayCenterPositions
