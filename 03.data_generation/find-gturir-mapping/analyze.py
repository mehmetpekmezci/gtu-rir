...
def origin_to_center_of_room(source,receiver,roomX,roomY):
    source=source-[roomX/2,roomY/2,0]
    receiver=receiver-[roomX/2,roomY/2,0]
    return source,receiver

def mesh_origin_to_center_of_room(roomX,roomY,mesh):
    mesh.vertices[:]=mesh.vertices[:]-[roomX/2,roomY/2,0]
...
def save_record(names[],sources[],recevivers[],mesh_filenames[]):
...
def main(record_dir):
  ...
  newmesh.export(data['maindir']+'/'+newmesh_filename)
  source=data['source']
  ...
  for i in range(6):
    for j in range(6):
       ...
       newsource=permutations(source[0],source[1],source[1],roomX,roomY,i,j)
       sources.append(newsource)
       newreceiver=permutations(receiver[0],receiver[1],receiver[1],roomX,roomY,i,j)
