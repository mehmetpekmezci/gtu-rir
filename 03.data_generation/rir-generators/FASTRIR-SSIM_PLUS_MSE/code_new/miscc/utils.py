...
def audio_ssim(data1,data2,mfcc_transform_fn):
   ...
   audio_ssim_value=ssim( mfcc1, mfcc2, data_range=2, size_average=True).item()
   #audio_ssim_value=loss1(mfcc1, mfcc2)
...
def pure_ssim_loss(real_data,generated_data):
         ...
         SSIM=ssim(generated_data_tiled,real_data_tiled, data_range=1, size_average=True).item()
         return SSIM
...
def compute_generator_loss(epoch,netD,real_RIRs, fake_RIRs, real_labels, conditions, gpus,mfcc_transform_fn):
    ...
    SSIM=pure_ssim_loss(real_RIRs,fake_RIRs)
    SSIM_LOSS=1-SSIM

    print("SSIM_LOSS:"+str(SSIM_LOSS))
    print("MSE_error1:"+str(MSE_error1))
    ...
    #MSE_COEF=0
    #SSIM_LOSS_COEF=1
    MSE_error = MSE_COEF*MSE_error1 + SSIM_LOSS_COEF*SSIM_LOSS
