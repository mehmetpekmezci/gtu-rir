...
class GANTrainer(object):
    ...
    def load_network_stageI(self):
        ...
        print("from model import MESH_NET")
    ...
    def train(self, data_loader, stage=1):
        ...
        for epoch in range(self.max_epoch):
            ...
            for i, data in enumerate(data_loader, 0):
                ...
                netG.zero_grad()
                mesh_net.zero_grad()
                errG,L1_error,divergence_loss0,divergence_loss1,divergence_loss2,divergence_loss3,divergence_loss4,divergence_loss5,MSE_error1,MSE_error2,criterion_loss= compute_generator_loss(epoch,netD,real_RIRs, fake_RIRs,real_labels, c_code,filters, self.gpus,self.mfcc_transform_fn,cfg)
                ...
                #if i % 100 == 0:
                if i % 100 == 0 :
                    print(f"saving model i : {i} epoch : {epoch}")
                ...
                if i % 1 == 0:
                    ...
                    if(epoch%self.snapshot_interval==0):
                        save_RIR_results(real_RIR_cpu, fake, epoch, self.RIR_dir,self.cfg)
                        if lr_fake is not None:
                            save_RIR_results(None, lr_fake, epoch, self.RIR_dir,self.cfg)
                    print('''[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f
                             Loss_real: %.4f Loss_wrong:%.4f Loss_fake %.4f   L1_error  %.4f 
                             Total Time: %.2fsec  i=%d
                          '''
                             % (epoch, self.max_epoch, i, len(data_loader),
                             errD.data, errG.data, 
                             errD_real, errD_wrong, errD_fake,L1_error*4096,(time.time() - start_t),i))
            end_t = time.time()
