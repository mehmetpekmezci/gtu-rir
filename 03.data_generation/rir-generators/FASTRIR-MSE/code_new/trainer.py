...
class GANTrainer(object):
    ...
    def plotWav(self,data,title,show=False,saveToPath=None):
     #plt.clf()
     plt.subplot(1,1,1)
     plt.plot(data)
     plt.title(title)
    ...
    def train(self, data_loader, stage=1):
        ...
        for epoch in range(self.max_epoch):
            print(str( datetime.datetime.now())+" : starting epoch : "+str(epoch)+"/"+str(self.max_epoch))
            start_t = time.time()
    ...
    def sample(self,file_path,stage=1):
        ...
        save_dir_GAN = cfg.EVAL_TARGET_DIR
        ...
        while count < num_embeddings:
            ...
            print("MP:num_embeddings:"+str(num_embeddings))
            ...
            #labels_embeddings_batch=(embeddings_batch+1)*5

            labels_embeddings_batch=embeddings_batch[:,10:]

            embeddings_batch = embeddings_batch[:,:10].astype(float)

            #print("MP:embeddings_batch:"+str(embeddings_batch))

            txt_embedding = Variable(torch.FloatTensor(embeddings_batch))
            
            
            #print("MP:txt_embedding:"+str(txt_embedding))
            print("AAAAAAAA MP:labels_embeddings_batch:"+str(labels_embeddings_batch))
            ...
            # MP: UYARI : BURAYI BATCH_SIZE ILE ESITLE
            #channel_size = 750 
            #channel_size = 16 
            channel_size = batch_size 
            ## BATCH_SIZE ile ayni olmali
            print("MP:channel_size:"+str(channel_size))
            
            for i in range(channel_size):
                fs =16000
                print("count="+str(count)+" i="+str(i)+" count+i="+str(count+i))
                print(labels_embeddings_batch[i]) 
                
                roomId=labels_embeddings_batch[i][0]
                configId=labels_embeddings_batch[i][1]
                speakerIteratonNo=labels_embeddings_batch[i][2]
                microphoneIteratonNo=labels_embeddings_batch[i][3]
                physicalSpeakerNo=labels_embeddings_batch[i][4]
                micNo=labels_embeddings_batch[i][5]

                record_name = f"SPEAKER_ITERATION-{speakerIteratonNo}-MICROPHONE_ITERATION-{microphoneIteratonNo}-PHYSICAL_SPEAKER_NO-{physicalSpeakerNo}-MICROPHONE_NO-{micNo}"

                wave_name=record_name+".wav"
#                graph_name=record_name+".png"
                save_dir = '%s/%s/%s' % (save_dir_GAN,roomId,configId)
                save_name_GAN = '%s/%s' % (save_dir,wave_name)
#                save_name_GAN_graph = '%s/%s' % (save_dir,graph_name)
                ...
                if  os.path.exists( save_name_GAN) :
                    print(save_name_GAN+" file already exists,...........................")
                
                ##print(np.shape(res['samples'])[0])
                
                ##print(res['rate'])
                
                print(save_name_GAN)

                ##print (np.array(res['samples']))
...
#                self.plotWav(rir_data[0],f"Sound Record : {save_name_GAN_graph}",saveToPath=save_name_GAN_graph)
