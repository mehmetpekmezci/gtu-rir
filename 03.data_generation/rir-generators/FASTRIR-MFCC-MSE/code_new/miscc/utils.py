...
def audio_mfcc_mse(data1,data2,mfcc_transform_fn):
   #print("data1.shape")
   #print(data1.shape)
   mfcc1 = mfcc_transform_fn(data1)
   mfcc2 = mfcc_transform_fn(data2)
   loss=nn.MSELoss()
   audio_mfcc_mse=loss( mfcc1, mfcc2)

   return audio_mfcc_mse 
     




def compute_generator_loss(epoch,netD,real_RIRs, fake_RIRs, real_labels, conditions, gpus,mfcc_transform_fn):
    criterion = nn.BCELoss()
    ...
    inputs = (fake_features, cond)
    fake_logits = nn.parallel.data_parallel(netD.get_cond_logits, inputs, gpus)
    MFCC_MSE_error = audio_mfcc_mse(real_RIRs,fake_RIRs,mfcc_transform_fn)
    #MP:MSE_error = loss(real_RIRs,fake_RIRs)
    #MP:MSE_error1 = loss1(real_RIRs,fake_RIRs)
    #SSIM=audio_ssim(real_RIRs,fake_RIRs,mfcc_transform_fn) 
    #SSIM_LOSS=1-SSIM
    #print("SSIM_LOSS:"+str(SSIM_LOSS))
    #print("MSE_error:"+str(MSE_error))
   
    #MP:MSE_COEF=0.2
    #MP:SSIM_LOSS_COEF=0.8
    #MP:MSE_error = MSE_COEF*MSE_error + SSIM_LOSS_COEF*SSIM_LOSS
    ...
    if(epoch<100):
         ...
         #errD_fake = criterion(fake_logits, real_labels) + 2* 4096 * SSIM_LOSS
         errD_fake = criterion(fake_logits, real_labels) + 2* 4096 * MFCC_MSE_error
    #MP:errD_fake = criterion(fake_logits, real_labels) + 5* 4096 * MSE_error1 + 40 * RT_error
    if netD.get_uncond_logits is not None:
        fake_logits = \
            nn.parallel.data_parallel(netD.get_uncond_logits,
                                      (fake_features), gpus)
    ...
    #MP:return errD_fake, MSE_error,RT_error
    #return errD_fake, SSIM_LOSS,RT_error
    return errD_fake, MFCC_MSE_error,RT_error
