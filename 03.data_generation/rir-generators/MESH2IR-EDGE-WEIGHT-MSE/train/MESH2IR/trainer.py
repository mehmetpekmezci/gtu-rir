    ...
    def train(self, data_loader, stage=1):
        ...
        for epoch in range(self.max_epoch):
            ...
            for i, data in enumerate(data_loader, 0):
                ...
                edge_weights = torch.from_numpy(np.array(data['edge_weights']))
                #print(data['edge_weight'])
                
                #data['edge_weights']= torch.from_numpy(np.array(data['edge_weights'],dtype=float))
                
                data.pop('RIR')
                data.pop('embeddings')
                data.pop('edge_weights')
                ...
                edge_weights = Variable(edge_weights).float()
                
                #data['edge_weights']=Variable(data['edge_weights']).float()
