...
def generate_ess_signal():
    ...
    parser.add_option('-r', '--rate', default=SAMPLE_RATE, type='int',
        metavar='HZ', help='Sample rate, default is 44100 Hz')
    parser.add_option('-I', '--fmin', default=MINFREQUENCY, type='float',
        metavar='HZ', help='Minimum frequency, default is 15 Hz')
    parser.add_option('-A', '--fmax', default=MAXFREQUENCY, type='float',
        metavar='HZ', help='Maximum frequency, default is 22 kHz')
    parser.add_option('-d', '--duration', default=TRANSMIT_DURATION, type='float',
    ...
    parser.add_option('-s', '--silence', default=2.0, type='float',
    ...
    ess_signal=full_array.astype('int16')
    #ess_signal=full_array.astype('float32')
    #ess_signal= np.fromstring(full_array, dtype="float32")
    

    #writer = wave.open(DATA_DIR+'/ess_signal.wav', 'wb')
    #writer.setnchannels(1) # 1 channel, mono
    #writer.setsampwidth(2) # 16bit
    #writer.setframerate(opts.rate) # sample rate
    #writer.writeframes(ess_signal)
