...
class GANTrainer(object):
    def __init__(self, output_dir):
        ...
        sample_rate = 16000 ;num_mfccs=40
        ...
        for epoch in range(self.max_epoch):
            ...
            for i, data in enumerate(data_loader, 0):
                ...
                errG,MSE_error,RT_error= compute_generator_loss(epoch,netD,real_RIRs, fake_RIRs,
                                              real_labels, c_code, self.gpus,self.mfcc_transform_fn)
    ...
    def sample(self,file_path,stage=1):
        ...
        while count < num_embeddings:
            ...
            print("MP:labels_embeddings_batch:"+str(labels_embeddings_batch))
            ...
            for i in range(channel_size):
                ...
                #record_name = f"RIR-MX-{labels_embeddings_batch[i][0]}-MY-{labels_embeddings_batch[i][1]}-MZ-{labels_embeddings_batch[i][2]}-SX-{labels_embeddings_batch[i][3]}-SY-{labels_embeddings_batch[i][4]}-SZ-{labels_embeddings_batch[i][5]}-DEPTH-{labels_embeddings_batch[i][6]}-WIDTH-{labels_embeddings_batch[i][7]}-HEIGHT-{labels_embeddings_batch[i][8]}-RT60-{labels_embeddings_batch[i][9]}"
                record_name = f"RIR-DEPTH-{labels_embeddings_batch[i][6]}-WIDTH-{labels_embeddings_batch[i][7]}-HEIGHT-{labels_embeddings_batch[i][8]}-RT60-{labels_embeddings_batch[i][9]}-MX-{labels_embeddings_batch[i][0]}-MY-{labels_embeddings_batch[i][1]}-MZ-{labels_embeddings_batch[i][2]}-SX-{labels_embeddings_batch[i][3]}-SY-{labels_embeddings_batch[i][4]}-SZ-{labels_embeddings_batch[i][5]}-{count+i}"
                wave_name=record_name+".wav"
                graph_name=record_name+".png"
                save_name_GAN = '%s/%s' % (save_dir_GAN,wave_name)
                save_name_GAN_graph = '%s/%s' % (save_dir_GAN,graph_name)
