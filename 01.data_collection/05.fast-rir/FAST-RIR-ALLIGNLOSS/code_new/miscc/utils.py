...
def compute_generator_loss(epoch,netD,real_RIRs, fake_RIRs, real_labels, conditions, gpus):
    ...
    ### ALLIGN START
    maxIndReal=array_of_max_point_index_within_first_1000_points_for_each_row_of_real_data=torch.argmax(real_RIRs[:,:,0:1000],dim=2)
    maxIndFake=array_of_max_point_index_within_first_1000_points_for_each_row_of_fake_data=torch.argmax(fake_RIRs[:,:,0:1000],dim=2)
    diffIndMax=(maxIndReal-maxIndFake)/2
    #print("diffIndMax")
    #print(diffIndMax)
    #print("array_of_max_point_index_within_first_1000_points_for_each_row_of_real_data="+str(array_of_max_point_index_within_first_1000_points_for_each_row_of_real_data))
    #print("array_of_max_point_index_within_first_1000_points_for_each_row_of_generated_data="+str(array_of_max_point_index_within_first_1000_points_for_each_row_of_fake_data))
    fake_rir_max=torch.max(torch.abs(fake_RIRs[:,0,0:1000]))
    real_rir_max=torch.max(torch.abs(real_RIRs[:,0,0:1000]))
    ratio_max=real_rir_max/fake_rir_max
    fake_RIRs=fake_RIRs*ratio_max


    
    clone_real_RIRs=real_RIRs.clone()
    clone_fake_RIRs=fake_RIRs.clone()

    for rowNumber in range(real_RIRs.shape[0]):
        real_RIRs[rowNumber,0]=torch.roll(clone_real_RIRs[rowNumber,0],int(-1*diffIndMax[rowNumber]))
        fake_RIRs[rowNumber,0]=torch.roll(clone_fake_RIRs[rowNumber,0],int(diffIndMax[rowNumber]))
    ...
    #maxIndFake=array_of_max_point_index_within_first_1000_points_for_each_row_of_fake_data=torch.argmax(fake_RIRs[:,:,0:1000],dim=2)
    #diffIndMax=(maxIndReal-maxIndFake)/2
    #print("2-array_of_max_point_index_within_first_1000_points_for_each_row_of_real_data="+str(array_of_max_point_index_within_first_1000_points_for_each_row_of_real_data))
    #print("2-array_of_max_point_index_within_first_1000_points_for_each_row_of_generated_data="+str(array_of_max_point_index_within_first_1000_points_for_each_row_of_fake_data))
    ## MP: BURADAKI PRINT TE FARKLI INDEX DEGERLERI DE CIKABILIR, ROLL YAPINCA index'i 1000'den sonraki  degerler de gelebilir, ki bunlarin arasindan yeni max'lar cikabilir.
        ...
        #print("ratio_max")
        #print(ratio_max)
        #print("torch.max(torch.abs(fake_RIRs[rowNumber,0]))")
        #print(torch.max(torch.abs(fake_RIRs[rowNumber,0])))
        #print("torch.sum(torch.abs(fake_RIRs[rowNumber,0]))")
        #print(torch.sum(torch.abs(fake_RIRs[rowNumber,0])))
        
        ###fake_RIRs[rowNumber,0]=fake_RIRs[rowNumber,0].clone()*ratio_max


        #print(torch.max(torch.abs(fake_RIRs[rowNumber,0])))
        #print(torch.max(torch.abs(real_RIRs[rowNumber,0])))
        #print(torch.sum(torch.abs(fake_RIRs[rowNumber,0])))
        #print(torch.sum(torch.abs(real_RIRs[rowNumber,0])))
